...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (session?.role === "user") router.replace("/user")
  }, [session, router])
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    const res = signIn(nik.trim(), password)
    if (!res.ok) {
          ...
          <form onSubmit={onSubmit} className="space-y-6">
            ...
            <motion.div variants={itemVariants} className="flex flex-col gap-3 pt-2">
              ...
              <Button
                ...
                className="w-full text-slate-400 hover:text-white hover:bg-white/5 h-11 disabled:opacity-50"
              >
                Gunakan Akun Demo
              </Button>
            </motion.div>
